...
mFPCA_score_test <- function(dat.id, n.ind.samp, samp.design, sparsity.level, 
  ...
  # mfaces package changed its attribute later 2022
  sum_of_scores          <- rowSums(mfaceObj$rand_eff$scores) # Edit by SK on Nov 9, 2022 while timing the code
  ...
  # mfaces package changed its attribute later 2022
  grp1_xi   <- mfaceObj$rand_eff$scores[g==1, , drop=FALSE] # Edit by SK on Nov 9, 2022 while timing the code
  grp2_xi   <- mfaceObj$rand_eff$scores[g==2, , drop=FALSE] # Edit by SK on Nov 9, 2022 while timing the code
